...
//algorithmus
		...
		int size= 256; //max size kosticky
		for (int i= 0; i < 3; i++) 
			if (dims0[i] > size) {
...
/*					IJ.showMessage("x "+x1+", y "+y1+", z "+z1,
					"Width "+(x2-x1)+", Height "+(y2-y1)+", Depth "+(z2-z1));
...
//pres scale				
	...
	//					imr.setVoxels(x1, y1, z1, dims[0], dims[1], dims[2], img);//test
						//	algoritmus
						for (int k = 0; k < nz; k++) {
	...
	//								double x111= pimg.get(i, j, k);
									...
									double ddx= (x002+x012+x022+x102+x112+x122+x202+x212+x222-x000-x010-x020-x100-x110-x120-x200-x210-x220)/(18.*dx);
									double ddy= (x020+x021+x022+x120+x121+x122+x220+x221+x222-x000-x001-x002-x100-x101-x102-x200-x201-x202)/(18.*dy);
									double ddz= (x200+x201+x202+x210+x211+x212+x220+x221+x222-x000-x001-x002-x010-x011-x012-x020-x021-x022)/(18.*dz);
		
//									imf.setVoxel(i, j, k, Math.sqrt(ddx*ddx+ddy*ddy+ddz*ddz));
...
//						double lattitude= Math.asin(d2[2]);
//						double longitude= Math.atan2(d2[1], d2[0]);
...
//pres scale										
										...
										for (int v= 0; v < n_sc; v++) {
											...
											if ((ii>=0)&&(ii<nx)&&(jj>=0)&&(jj<ny)&&(kk>=0)&&(kk<nz)) {
												double val= pimh.get(0, ii, jj, kk, v)*e1[0]+pimh.get(1, ii, jj, kk, v)*e1[1]+pimh.get(0, ii, jj, kk, v)*e1[2];
												val= val*sigmav;
												if (val < min) min= -val;
											}
...
//					ImageStack imh= imf.crop(x3-x1, y3-y1, z3-z1, x4-x3, y4-y3, z4-z3); //by crop
/*					IJ.showMessage("Bitdepth: "+imh.getBitDepth()+", Width"+imh.getWidth()+", Height"+imh.getHeight()+", Depth"+imh.getSize(),
							"Width"+(x4-x3)+", Height"+(y4-y3)+", Depth"+(z4-z3));
*/
					imf.getVoxels(x3-x1, y3-y1, z3-z1, x4-x3, y4-y3, z4-z3, img1); //by getvoxels
					imr.setVoxels(x3, y3, z3, x4-x3, y4-y3, z4-z3, img1);
					//pocitadlo
					if (numbox > 1) IJ.showStatus("Done "+String.format("%.2g",(iz + nums[2] * (iy + ix * nums[1]))/(double)(numbox - 1)));
					//IJ.showProgress(iz + nums[2] * (iy + ix * nums[1]), numbox - 1);
				}
			}
		}
